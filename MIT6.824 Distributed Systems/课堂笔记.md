...
给落后的follower发送InstallSnapshot的时机：Leader想发给Follower的next log entry被舍弃了，于是只能发snapshot了。
...
解决方案：读取的时候，想更新memcache需要有lease；那么，只要在收到delete(key)的时候，将对应的lease从表中删掉，就可以禁止老的读操作更新memcache。
