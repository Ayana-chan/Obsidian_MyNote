...
网站内部会维护一个TC系统用于监视。它定期向certificate log service索取日志的拷贝（或者上次访问过后新加的所有日志）。由于TC是由网站自己维护的，因此它知道自己有哪些证书，也就能通过比对发现伪证书。

>TC可能是网站方自己维护的，也可能是CA代表客户进行维护的，或者交给第三方监视系统（需要把域名和相关有效证书提供过去），因此TC都知道哪些证书是合法的。

由于浏览器强制要查看certificate log service，而certificate log service公开，所有TC都能进行检查，因此所有证书都被公开地进行审计。
