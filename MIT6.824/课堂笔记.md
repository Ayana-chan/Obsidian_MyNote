...
如果W=1，R=N，则为WARO(Write All Read one)机制。
...
（严格）排它锁要最后才能释放的目的：防止脏读（因为读到的数据必然已被其他事务提交），也能进一步解决级联回滚问题。
...
7. TC根据标决结果：
	1. 若全为YES，则向各个参与者发送COMMIT消息，让其提交事务，并得到确认。参与者将事务持久化存储。
	2. 否则，向各个参与者发送ABORT消息，让其撤回事务。
