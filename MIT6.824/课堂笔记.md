...
但是EBS会不断发8KB大的page，不如发log entries。但这就要求处理的每个replica都知道日志条目是什么意思，而不能是单纯的存储系统。这就是针对应⽤程序的存储系统（application specifific storage system）。Aurora的这个优化使得性能提高了35倍。
