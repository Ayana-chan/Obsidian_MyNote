...
解决各种<u>组合或排列</u>的**最优解值问题**的时候(比较经典的完全背包), 就需要求完一个dp值的最优解, 然后才能走向下一个dp; 因此要对每个dp都遍历一遍可取值, 从而转移所有合法的上一状态, 并获得局部最优解. 因此遍历顺序和求排列的总数时一致. [322. 零钱兑换 - 力扣（LeetCode）](https://leetcode.cn/problems/coin-change):
```cpp
...
略加变形的问题: [LeetCode 139. 单词拆分](https://leetcode.cn/problems/word-break/), 下面两种解法主要思路都是尝试所有可行后缀, 有一个成功就算成功：
```cpp
