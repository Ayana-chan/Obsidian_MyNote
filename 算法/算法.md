...
不同的扩展结点选取法:
1. **队列式（FIFO）分支界限法**: 将活结点表组成一个<u>队列</u>，并按队列的先进先出原则选取下一个结点为当前扩展结点。
2. **优先队列式分支界限法**: 将活结点表组织成一个<u>优先队列</u>，并按优先队列中规定的结点优先级选取**优先级最高**的下一个结点成为当前扩展结点。
...
## Minimum Cost Network Flow Problem TODO
