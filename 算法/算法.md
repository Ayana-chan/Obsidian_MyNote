...
而只要$Q$满足了<u>条件2</u>, 就是**NP难(NP-hard)问题**. 因此NPC问题就是可多项式验证的NP-hard问题, 即$NPC = NP \cap NP\mbox{-}hard$.
...
每个优化问题都有<u>对应的判定问题</u>: 此问题下是否存在一个可行解$y$, 其对应值比目标值$K$更优. 
...
- $m_Q$是一个**评估函数**: 对于每个二元组$(x,y)$, 其中$x \in I_Q$, $y \in SOL_Q(x)$, $m_Q(x,y)$为可行解$y$的**值**.
...
假设$Q$是一个<u>NPO问题</u>, 如果存在常数$r \geq 1$, 使得$Q$有一个<u>多项式时间</u>的<u>r-近似算法</u>, 则称$Q$属于**APX类**.
...
假设$Q$是一个NPO问题, 如果对于Q的任意实例$x$和任意有理数 $r > 1$, 算法$A$以$(x, r)$为输入时, 在关于$|x|$的多项式时间内输出 r-近似解, 则称算法$A$是$Q$的**多项式近似方案(PTAS, Polynomial-time approximation schemes)**. 若问题$Q$存在PTAS, 则称$Q$属于**PTAS类**.

> [!example]
> 背包问题属于PTAS类, S.Sahni 算法是背包问题的一个PTAS.
