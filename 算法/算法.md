...
每个优化问题都有<u>对应的判定问题</u>("此问题下是否有比该值<u>更优</u>的解"); 一个优化问题的算法通常可用来求解对应的判定问题. <u>判定问题不会比对应的优化问题更难</u>.
...
对于任意一个优化问题$Q$，如果$Q$属于**NPO类**，则$Q$对应的<u>判定问题</u>$Q_D$一定是**NP问题**. 
...
如果一个NPO问题$Q$的<u>判定问题</u>$Q_D$是<u>NP-hard问题</u>, 则$Q$是**NP-难优化问题(NP-hard optimization problem)**.
...
给定一个<u>优化问题</u>Q和一个<u>多项式时间</u>的近似算法A, 如果对Q的任何实例x:
