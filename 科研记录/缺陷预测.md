...
TabPFN在44数据集上性能比KSETE好, 但是会出现预测概率全部小于0.5导致输出全0的情况. 此时的AUC依然特别高, 也许改改threshold有可能更好用?

ICML 2023: XTab: Cross-table Pretraining for Tabular Transformers
- XTab可以跨表预测, 对特征是否相同没有要求. 基于Transformer.
- 性能不如2018年的CatBoost, 且实验当中CatBoost比XGBoost<u>稍微</u>好一点.
- 相关工作里面把TabPFN也放在跨表迁移学习部分了, 不知道为什么
- 说TransTab不能跨领域, 而XTab可以

arxiv 2018: CatBoost: Gradient Boosting with Categorical Features Support
- TODO
...
Proceedings of the IEEE, 2020: A Comprehensive Survey on Transfer Learning
- TODO
