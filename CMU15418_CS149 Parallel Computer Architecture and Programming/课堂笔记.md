...
- 进行物理模拟时，内存多了才能模拟更复杂、更大的场景，因此的**MC**，追求使用更多处理器（更多内存）来解决更大的问题，并希望效率也尽可能提高。
...
> [!note]
> 下面似乎都假设**线性内存模型**. 例如链表的情况下, 需要保证写入的时候要保证<u>新结点的构造</u>不落后于<u>前一个结点的next指针的修改</u>, 否则对读者来说next指针可能是个野指针.
...
在一些情况下, 会认为程序掌握整个机器, 因此可以认为每个线程都被分配到独占的核上一直运行, 那么这个时候小锁可能比无锁还有点儿优势.
