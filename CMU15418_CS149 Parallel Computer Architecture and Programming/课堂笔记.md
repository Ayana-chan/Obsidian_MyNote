...
若work queue中每个task大小不一，那么，**优先分配大任务**是最好的，因为这样分配到最后的时候，各线程的任务差距就不会很大，不会发生到末尾的时候一个线程突然领了个特别大的任务的情况。
...
通过**fusing loops**暴力压行、强拆模块，把遍历的一个单位的操作都放在一起, 可以减少内存读写次数，增加计算强度, 同时增强locality。如果不是数组计算的话应该优化不太大，因为单个数字直接放寄存器里就行了，但大数组的话遍历完一遍之后寄存器和cache基本等于全部失效。（猜测：也许纯函数编程或者说data parallel能解决一部分问题）
...
> [!note]
> 因此, "乱序"情况来自:
> - CPU自己的乱序执行, 在其他线程看来它是真乱的.
> - CPU使用write buffer进行加速(掩盖内存操作延迟), 会出现内存操作随机重排.
> - 内存修改同步速度不一定, 多个变量的修改操作可能以不同顺序被探知(IRIW问题).
...
- Total Store Ordering (TSO) 允许在一次写没完成的时候就进行读, 这相当于允许 <u>FIFO write buffer</u>.
- Partial Store Ordering (PSO) 相当于对一次写之后不进行任何约束, 这相当于<u>无FIFO的 write buffer</u>.
...
- 线程内程序本身有 program order (po)
- 线程间对同一个变量的读写构成 dependence order (do)
...
- 其他线程后来的require会同步自己的release.

![](assets/1737296805342.png)

> [!note]
> 这图意思应该是: 在2和3之前, 会执行require, 读到其他release所同步的操作; 而release同步的是1和2的操作.

[Cpp Memory Order](Cpp/Cpp.md#Memory%20Order)
...
P0只有写操作, 因此只需要加SFENCE; 同理P1只有读操作, 只需要加LFENCE.
- SFENCE构建了`A=1 -> Ready=1`的因果.
- LFENCE要求当已经读到`Ready=1`的时候, 此后必须发生`A=1`.
...
- 某一行想传播到某一列时，就把指定的交叉路由器激活，使得横向信号只在那个点上触发转发，变成竖直信号。
- 这也使得不同行之间的转发不会冲突。
...
**Cut-through**: 当接收完一个包的头时, 如果判断其能继续发送(下一跳link可用), 那么就<u>直接开始转发</u>此包(流式转发), 而不需要等待包被自己完全接收; 否则完全接收此包到buffer内.
...
使用**test-and-set**汇编指令（ts）来实现lock.
...
- 数组的每一项都要进行padding，使得每一项都在不同的cache line里面。
...
- 相比centralized barrier, 增加了一个私有变量, 减少了一个spin.
![](assets/Pasted%20image%2020250315002150.png)

![](assets/Pasted%20image%2020250315003111.png)

使用combining tree来实现barrier, 使得对锁的竞争减少：
- 一个子树完成barrier后再向上提交.
- root发现全部完成后，向下广播通知以解锁。
![](assets/Pasted%20image%2020250315003300.png)

如果有的情况下, 并发量很低, 极大概率不会出现两个线程访问同一个临界资源的情况. 那么如何在降低锁消耗的同时, 防止偶然出现的竞争导致的错误? 
...
- `addr`: 目标地址.
- `compare`: 预计的目标值.
- `val`: 如果目标值与预期吻合, 则让其更新为此值.
- 总是返回此地址的之前的值, 以让上层检查是否swap成功.

使用CAS实现`atomic_min`, 则需要每次都限制当前的`addr`必须是`old`, 因为只有它是`old`的时候, `min`的计算结果才是正确的, 或者说相当于`addr`的读取到计算到写入没有被打断.
![](assets/Pasted%20image%2020250316021625.png)

> [!note]
> 同时的大量的CAS会降低性能, 甚至不如内核态锁.

注: CAS有ABA问题.

`atomic<T>`可以对任何数据结构进行原子化. 基本数据结构会使用真实的原子操作(一些汇编命令)来实现, 而复杂数据结构会使用大锁(不过会尽可能优化). 还能使用`is_lock_free`方法来检查目标数据结构是否是lock-free的.
![](assets/Pasted%20image%2020250316021913.png)

这些基本类型的原子操作(x86里面就是汇编加个*lock前缀*)是通过给cache协议添加新功能来实现的, 主要功能就是**在写完成之前不允许其他核的任何访问**, 例如锁住cache line, 锁住bus或保持NACK响应. 注: 不同原子变量的内存发生重合是UB.
![](assets/Pasted%20image%2020250316023221.png)

## 细粒度锁数据结构

若要对一个有序链表进行并发的插入或删除操作, 那么最暴力的解决方案是给整个list加大锁, 只要开始操作就上锁.

**hand-over-hand**: 给每个结点上一个锁, 操作需要两个锁; 往右遍历的时候, 先释放左边的锁, 在求取新的右边的锁, 像爬绳子一样往右推进. 
- 后进入操作的线程就会被先入的线程挡住. 
- 依然要在List处放一个锁, 以单纯处理进入时候的初始行为(比如同时拿到两个锁). 
- 因为链表的<u>更新很快</u>, 所以用spin lock会更快(pthread lock对于小节点来说太重了).
![](assets/Pasted%20image%2020250316030657.png)

![](assets/Pasted%20image%2020250316030805.png)

## 无锁数据结构

只要使用了锁了就叫blocking data structure, 因为无论如何都会因为线程上锁后被swap导致其他线程也无法继续执行.
![](assets/Pasted%20image%2020250316032119.png)

![](assets/Pasted%20image%2020250316121040.png)

> [!note]
> 下面似乎都假设**线性内存模型**. 例如链表的情况下, 需要保证写入的时候<u>新结点的构造</u>不落后于<u>前一个结点的next指针的修改</u>, 否则对读者来说next指针可能是个野指针.

### 单读单写FIFO无锁队列

**单读者单写者FIFO带界限队列**(循环队列):
- 头指针head指向最旧元素(即下一步读取时要弹出的).
- 尾指针tail指向最新元素的下一个元素(即下一步要插入的). 
- 当尾指针的下一个元素就是头指针时, 声明队列已满(两个指针重合才表示空队列, 从而能够区分). 因此其容量是$N-1$.
完全不需要任何原子操作和锁, 不会出现非法情况. head被读取线程拥有, tail被写入线程拥有, 二者对对方的指针仅仅是读取, 且显然读到过老的head或tail都不会出现非法或不一致的情况.
![](assets/Pasted%20image%2020250316122707.png)

**单读者单写者FIFO无界限队列**(链表):
- 头指针head指向最旧元素的上一个元素.
- 尾指针tail指向最新元素.
- head和tail重合表示空队列.
- reclaim指向物理头结点, head指向实际头结点(总是有一个空节点作为链表头). $[reclaim, head)$中的节点都是**非法**的.
- 写者操作reclaim和tail, 读者操作head.
- 只有写者能进行内存分配和删除操作.
- 读取节点的时候, 移动head即可; <u>添加节点</u>的时候, 在于tail后面添加数据的同时, 把$[reclaim, head)$的节点全都<u>删除</u>.
![](assets/Pasted%20image%2020250316135853.png)
![](assets/Pasted%20image%2020250316140618.png)

### 无锁栈及其修复

一个**有问题**的**无锁栈**(链表), 它通过CAS保证只有栈顶结点(地址)符合预期时, 才真正进行push(保证新结点的next合法)和pop(保证新栈顶是合法节点). 
![](assets/Pasted%20image%2020250316143447.png)

会出现**ABA问题**. CAS保证了栈顶没变, 但这不意味着栈未被修改. 极端方式下栈发生修改之后栈顶元素依然没变, 那么CAS中的线程就会误认为自己CAS成功了, 然后做了非法操作. 
- 例如, 如果一个线程进行pop, 在进入CAS的时候, 另一个线程连续完成了pop A, push D, push A(还是原来的地址), 那么原线程会误认为自己CAS成功, 导致连续pop了A和D两个元素(正确行为是只pop A).
![](assets/Pasted%20image%2020250316143716.png)

解决方案是使用`pop_count`, 保证栈本身没有发生任何pop, 满足条件的同时就会跟着原子地进行pop操作. 由于`pop_count`和栈顶指针无关, 因此不得不使用**DCAS(double CAS)** 或**doubleword CAS**进行两个变量的同时CAS.
- 只要没发生过pop, 那就不可能出现ABA问题.
- 刚好在pop操作内也要调整`pop_count`, 因此恰好让`pop_count`通过CAS进行操作.
![](assets/Pasted%20image%2020250316144540.png)

DCAS的参数太多了, 硬件难以支持, 因此更长字的CAS会更普遍, 对一段连续内存(相当于多个变量)进行CAS. `cmpxchg16b`就可以支持64位的两个指针.
![](assets/Pasted%20image%2020250316145219.png)

还有一个问题是**reference freed memory**: pop里面的 `Node *new_top = top->next`的时候, top指针的目标可能已经被另一个pop给删除了, 因此`next`已经是非法内存域.

解决方案是 **hazard pointers(风险指针)**: 
- 是一个指针数组, 每个线程分有一个元素, 相当于一个可被其他线程读取的local变量.
- 在这个数组里的所有指针都禁止被delete, 以保证各线程的操作合法性. 因此线程会在指针有可能被其他线程delete的时候, 将其放入此数组以进行保护.
- 如何进行保护(或者说如何处理delete)在此处没有给出.
![](assets/Pasted%20image%2020250316150009.png)

### 无锁链表

一个只进行插入(指定某一个结点, 在其后面插入)的无锁链表, 可以通过CAS保证新结点的next是合法的.
![](assets/Pasted%20image%2020250316153903.png)

但如果有删除的话, 复杂度就大大增加(此处没给出解决).
![](assets/Pasted%20image%2020250316154343.png)

## 总结

小锁(上面两条线)在低冲突时有多余消耗, 高冲突时流量过大; 而无锁(下面两条线)表现越来越好.
![](assets/Pasted%20image%2020250316154922.png)

在一些情况下, 会认为程序掌握整个机器, 因此可以认为每个线程都被分配到独占的核上一直运行, 那么这个时候小锁可能比无锁还有点儿优势，因为可以避免线程切换导致其他线程阻塞的问题，而无锁编程往往使用乐观锁进行多次带消耗的尝试。
![](assets/Pasted%20image%2020250316155753.png)

- 小锁相比大锁能获得更多的并发度, 但依然有性能损耗, 并且代码复杂.
- 无锁太过有技巧性, 并且要考虑memory order问题.
- 此外要注意, 无锁不能消除冲突, 依然可能爆发大冲突卡顿.
![](assets/Pasted%20image%2020250316160030.png)
