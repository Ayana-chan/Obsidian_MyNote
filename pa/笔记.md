...
>切换ARCH的时候，尽量把所有程序都update一下，否则已经编译好但没有重新编译的程序的ehdr.e_machine是错误的！
...
`navy-apps/libs/libam/Makefile`中，将若干底层的文件软链接过来：
```
  5   $(info Setup link to header files)
  6   $(shell ln -sf -T $(AM_HOME)/am/include/am.h include/am-origin.h)
  7   $(shell ln -sf -T $(AM_HOME)/am/include/amdev.h include/amdev.h)
  8   $(shell ln -sf -T $(AM_HOME)/klib/include/klib.h include/klib.h)
  9   $(shell ln -sf -T $(AM_HOME)/klib/include/klib-macros.h include/klib-macros.h)
```

而`navy-apps/libs/libam/include/am.h`也include了`am-origin.h`，也就是`$(AM_HOME)/am/include/am.h`。而am.h会include `amdev.h`。

因此，libam内的代码是对am头文件的新实现，而头文件中定义的东西都是可以直接用的，比如各种宏。而am的实现和应用会按需调用klib.h和klib-macros.h，`io_read`、`AM_xxx_T`等宏也是可以用的。

而`navy-apps/libs/libam/include/navy.h`此时成为了`ARCH_H`，被am.h include。因此，要用到的通用navy库应当写在navy.h里面。

于是直接把`abstract-machine/am/src/platform/nemu/ioe`下的所有文件都拷贝到`navy-apps/libs/libam/src`中，然后把每个实现函数修改成navi api即可。

应该尚未链接复杂库（但有`LIB_DEP = libndl`，可能可以用ndl，但`include <NDL.h>`报错），只能用简单库函数。

键盘按键的enum，目前目标是从按键名字映射到对应的enum：
```c
  4 #define _KEYS(_) \
  5   _(ESCAPE) _(F1) _(F2) _(F3) _(F4) _(F5) _(F6) _(F7) _(F8) _(F9) _(F10) _(F11) _(F12) \
  6   _(GRAVE) _(1) _(2) _(3) _(4) _(5) _(6) _(7) _(8) _(9) _(0) _(MINUS) _(EQUALS) _(BACKSPACE) \
  7   _(TAB) _(Q) _(W) _(E) _(R) _(T) _(Y) _(U) _(I) _(O) _(P) _(LEFTBRACKET) _(RIGHTBRACKET) _(BACKSLASH) \
  8   _(CAPSLOCK) _(A) _(S) _(D) _(F) _(G) _(H) _(J) _(K) _(L) _(SEMICOLON) _(APOSTROPHE) _(RETURN) \
  9   _(LSHIFT) _(Z) _(X) _(C) _(V) _(B) _(N) _(M) _(COMMA) _(PERIOD) _(SLASH) _(RSHIFT) \
 10   _(LCTRL) _(APPLICATION) _(LALT) _(SPACE) _(RALT) _(RCTRL) \
 11   _(UP) _(DOWN) _(LEFT) _(RIGHT) _(INSERT) _(DELETE) _(HOME) _(END) _(PAGEUP) _(PAGEDOWN)                                                                                                              
 12 
 13 #define enumdef(k) SDLK_##k,
 14 
 15 enum SDL_Keys {
 16   SDLK_NONE = 0,
 17   _KEYS(enumdef)
...
可以由以下组合获取（思路来自于miniSDL）。查找名字在keyname中的下标号，其对应的就是enum的值。
```c
  3 #define keyname(k) #k,
  4 
  5 static const char *keyname[] = {
  6   "NONE",
  7   AM_KEYS(keyname)
  8 };
```

若想跑在native上，`__am_gpu_config`中必须添加：
```c
cfg->present = true;
cfg->has_accel = false;
cfg->vmemsz = 0;
...
execve如果成功的话，当前代码就不会继续跑下去了，因此成功的时候无需返回。
...
execv(), execvp(), execvpe()
       The char *const argv[] argument is an array of pointers to null-terminated strings that represent the argument list available to the new program.  The first argument, by  convention,  should
...
p - execlp(), execvp(), execvpe()
       These functions duplicate the actions of the shell in searching for an executable file if the specified filename does not contain a slash (/) character.  The file is sought in the colon-sep‐arated list of directory pathnames specified in the PATH environment variable.  If this variable isn't defined, the path list defaults to a list that includes  the  directories  returned  by confstr(_CS_PATH) (which typically returns the value "/bin:/usr/bin") and possibly also the current working directory; see NOTES for further details.
...
终极拷问：当你在终端键入`./hello`运行Hello World程序的时候, 计算机究竟做了些什么? 

1. 终端解析出程序路径和参数（此处为空参），然后调用系统调用**execve**，传入程序路径和参数。
2. 系统调用时，会设置异常号、记录参数、保存上下文，然后再根据异常号来进入系统调用处理程序（处于内核），并通过a7寄存器来识别系统调用信息，而a0、a1、a2寄存器作为系统调用的参数。
3. hello程序的执行内容被**复制**到指定的内存区域，然后PC被设为该程序的起始位置。其中“Hello World”字符串数据被存储在`.rodata`中。
4. 程序开始执行，然后到**printf**库函数处。库函数printf事先使用open系统调用来将串口作为文件打开，并获得其文件标识符。printf会在malloc申请的缓冲区记录待输出数据，然后将其传给**write**系统调用，write的参数分别设为串口的文件标识符、待输出数据的起点指针、待输出数据的长度。
5. write会调用**串口设备所对应的写函数**，将待输出数据的每个字符都逐一传给*由AM的trm模块提供的运行时环境中的putch函数*。
6. **putch**函数会将单个字符的数据传给串口所映射的内存区域，从而将字符信息输出给串口。
7. **串口设备**进行实际的输出。
8. 程序成功执行到末尾，会执行系统调用**exit**，这将会重新装载终端程序，正如同装载hello程序一样。
