...
而`navy-apps/libs/libam/include/am.h`也include了`am-origin.h`，也就是`$(AM_HOME)/am/include/am.h`。而am.h会include `amdev.h`，相当于能访问到大部分的am api。
```c
#include "am-origin.h"
```

因此，libam内的代码是对am头文件的新实现，而头文件中定义的东西都是可以直接用的，比如各种宏。而am应用自己会调用klib.h和klib-macros.h，因此io_read等宏也是可以用的。而`navy-apps/libs/libam/include/navy.h`此时成为了`ARCH_H`，被am.h include。因此，要用到的navy库应当写在navy.h里面。
