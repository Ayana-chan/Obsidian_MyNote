...
BITS取出x的二进制表示中高至hi位、低至lo位的数。而SEXT能将x的低位截取后转化成uint64_t类型。
...
bge等B型命令后面加z则表示和zero 0进行比较，都是伪指令，完全可以由bge实现。

TODO：`INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh , S, Mw(src1 + imm, 2, src2 & 0xffff));` 这个`& 0xffff`应该不需要。

`INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra     , R, R(dest) = (word_t)(((int32_t)src1) >> (src2 & 0x1f))); ` 为什么要这样转化类型，是因为要让空位用 `x[rs1]`的最高位填充吗？包括srai
