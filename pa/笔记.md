...
若f1.c和f2.c都include了tf.h，如果在头文件中用宏定义一个过程的话，就可以让f1使用此宏来访问f2的函数
# pa0
...
如果定义了宏`__NATIVE_USE_KLIB__`，那么klib的所有实现（即`.c`文件）才能被加载；由于klib的函数与对应的glibc里的函数是完全一模一样的，而本地代码的优先级高于标准库，因此会优先使用klib的实现。
