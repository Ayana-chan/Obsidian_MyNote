之所以大量使用链表，我猜是如下理由：
- 不能把链表直接改成数组：每个结构体所占空间应当是确定的，且数组长度在定义时没有定论。
- 不能指向一个外部（上级）数组：没有gc，内存释放起来很麻烦，甚至可能不得不要再设一层结构体，得不偿失。
## ifnet
...
`sa_data`是结构体的最后一个成员，可边长。但由于`sa_len`只占一个字节，因此`sa_data`的长度也不应超过254字节（带上`len`和`family`为256字节）。
