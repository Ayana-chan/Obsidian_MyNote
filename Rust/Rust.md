...
- Fn: 调用时对闭包进行不可变借用，适用于对捕获的变量进行不可变借用的场景。
- FnMut: 调用时对闭包进行可变借用，适用于对捕获的变量进行可变借用的场景（也因此这个闭包无法并发调用）。
- FnOnce: 调用时拿取闭包的所有权，适用于对捕获的变量进行夺取所有权的场景（也因此这个闭包只能调用一次）。
- `Fn` 是 `FnMut` 的子 trait，`FnMut` 是 `FnOnce` 的子 trait。

闭包的trait写法形如`Fn(i32) -> i32`。例如下面的函数可以接受这种闭包作为参数：
```rust
struct STName<T> 
where T: Fn(i32) -> i32 {
	...
