...
定义1.3.2 设$X_1,X_2,\cdotp\cdotp\cdotp,X_n$ 是相互独立的随机变量，且$X_i\sim N(0,1)(i=1$,

$2,\cdotp\cdotp\cdotp,n)$,则称随机变量
...
$\chi^2(n)$的概率密度为：
$$f(x)=\begin{cases}\frac{1}{2^{\frac{n}{2}}\Gamma\left(\frac{n}{2}\right)}x^{\frac{n}{2}-1}\mathrm{e}^{-\frac{x}{2}},&x>0\\\\0,&x\leqslant0\end{cases}$$
# 特殊函数
