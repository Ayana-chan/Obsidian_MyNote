...
> 
> 如果$\mu$和$\sigma^2$<u>都未知</u>，则$w$会拥有面积，是有内点的，可以求得完全充分统计量：
> $$T(x)=\left(\sum_{i=1}^nx_i,\sum_{i=1}^nx_i^2\right)$$
>  
> 但是，当$\mu$或$\sigma^2$的<u>其中一个已知</u>的时候，$w$的值域变成了直线，**没有内点**，因此**无法使用**该方法求此时正态分布的完全充分统计量。
...
1. 若$h(T(x))$是$q(\theta)$无偏统计量，则$h(T(x))$也是$q(\boldsymbol{\theta})$的UMVUE。即寻找完全充分统计量的函数使之成为$q(\boldsymbol{\theta})$的无偏估计。（凑形式）
2. 若能获得$q(\theta)$的一个无偏估计量$\varphi(x)$,则$E(\varphi(x)|T(x))$就是$q(\theta)$的UMVUE。（暴力求）

> [!note]
> 因此，要求UMVUE时，基本就要先求完全充分统计量，然后用Lehmann-Scheffe获取UMVUE。
...
设分布族为$\{P_{\theta},\theta\in\boldsymbol{\theta}\}$，密度函数为$p(x,\theta)$，$\Theta$为直线上的一个开区间。满足下述条件的分布族$\{P_\theta,\theta\in\boldsymbol{\theta}\}$称为**Cramer-Rao族**：
...
当<u>仅有1成立</u>时，我们可以定义所谓的**Fisher信息量（Fisher Information Number）**：
$$I(\theta)=E\Big[\frac\partial{\partial\theta}\ln p(x,\theta)\Big]^2(0\leq I(\theta)\leq\infty)$$

> [!tip]
> 里面的$\frac\partial{\partial\theta}\ln p(x,\theta)$就是[极大似然估计](概率论与数理统计/概率论与数理统计.md#极大似然估计)的时候所用的得分函数。因此Fisher信息量可以看做是得分函数的二阶矩。
...
如果$X_1,X_2,...,X_n$是来自总体的样本，可以证明$I(\theta)=nI_1(\theta)$,其中$$I_1(\theta)=E(\frac\partial{\partial\theta}\ln p(X_1,\theta))^2$$

> [!tip]
> 这说明样本增加时，信息量线性增加。

若将统计量$T(x_1,x_2,\dots,x_n)$的密度函数或分布列记为$p_T(t;\theta)$，则它所包含的的信息量为:
$$\text{}\\I_T(\theta)=E_\theta\left[\frac\partial{\partial\theta}\ln p_T(t;\theta)\right]^2$$
