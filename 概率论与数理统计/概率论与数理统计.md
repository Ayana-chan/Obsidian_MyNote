...
0,&L(x)<k
\end{cases}$$
其中常数$k$依然由$E_{\theta_0}(\varphi(x)) = P_{\theta_0}\{L(x)\geq k\}=\alpha$确定。

> [!info]
> 此时很可能有$P_{\theta_0}\{L(x)=k\}>0$。于是，假设当$L(x) = k$的时候有$\varphi(x)=r$，需要满足<u>式3</u>，因此有：
...
> [!tip]
> 当$P_{\theta_0}\{L(x)>k\} = P_{\theta_0}\{L(x)\geq k\} = \alpha$时，有$r=1$，使得离散时的$\varphi(x)$也可以写成连续时的形式（反之，连续时的情况天然可以代入离散时的式子）。
