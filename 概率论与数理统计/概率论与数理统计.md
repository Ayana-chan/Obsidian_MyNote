...
**唯一性**：设参数$q(\theta)$是可估的，且$T(x)$ 和$S(x)$都是$q(\theta)$的UMVUE，则对所有的$\theta\in\Theta$, 有 $P_\theta \{ T( x) = S( x) \} = 1$，即UMVUE在概率$1$下是**唯一的**。
...
> 
> 换个角度来说, 把总体分布函数处理好后有时能直接看出完全充分统计量是什么.
...
> [!note]
> $T(x)=E_\theta(\varphi(x)|S(x))$就是完全统计量版本的$U_q^T$, 因此它必然是完全充分统计量$S(x)$的函数.
...
> [!tip]
> 如果$p$是联合分布的话, 取$ln$后会变成简单相加, 因此同分布的时候只需计算其中一个的二阶导数即可. 
> 
...
通常称$\frac1{I(\theta)}$为**Cramer-Rao下界**(C-R下界)。
...
一定有$\Theta_0 \cap \Theta_1 = \emptyset$, 但不要求$\Theta_0 \cup \Theta_1 = \Theta$.
...
> [!info]
> 使用$z=\frac{\overline{x}-\mu_{0}}{\sigma_0/\sqrt{n}}$作为统计量的称为**z检验**.
...
设在<u>此检验问题</u>中：设$\varphi^*(x)$是水平为$\alpha$检验, 如果对任一水平为$\alpha$的检验$\varphi(x)$, 有
...
对假设检验问题：$$H_0{:}\theta=\theta_0;H_1{:}\theta>\theta_0$$上述定理<u>全部成立</u>。
...
对某些检验问题, 虽然不存在UMPT, 但存在UMPUT. 因为UMPUT仅在无偏检验范围内取最优.
...
> [!info] 贝叶斯估计的优缺点
> **优点**:
> 1. 因为所有的参数都是随机变量，因此更容易构建复杂模型.
> 2. 利用先验信息，结合似然原则，统计推断更精准.
> 
> **缺点**:
> 1. 具有主观性，与科学的客观性相矛盾.
> 2. 随着数据量的增加，先验的影响力会越来越小.
> 
> 注：如果先验是均匀分布，则贝叶斯方法等价于经典方法.


## 贝叶斯网络

- 全局语义将完整联合分布定义为局部条件分布的乘积。
- 局部语义：每个节点在给定其父节点的情况下与其非后代节点条件独立。

每个节点在给定其马尔可夫毯的情况下，与其他所有节点都是条件独立的，马尔可夫毯包括该节点的父节点、子节点以及子节点的父节点。

构造贝叶斯网络，需要一种方法，通过一系列局部可检验的条件独立性声明来确保所需的全局语义。

对于$i$从$1$到$n$, 将$X_i$添加到网络中. 从$X_1,\ldots,X_{i-1}$(**自己前面的结点**)中选择父结点，使得$P(X_i|Parents(X_i))=P(X_i|X_1,\ldots,X_i-1)$, 其中$X_i$的父结点还可表示为$Pa(X_i)$或$\pi(X_i)$.

选择这些父结点能够确保**全局语义的正确性**:
$$\begin{aligned}
P(X_1,\cdots,X_n)
&=\prod_{i=1}^nP(X_i|X_1,\cdotp\cdotp\cdotp X_{i-1}) \quad\text{(链式法则)}\\
&=\prod_{i=1}^nP(X_i|Parents(X_i)) \quad\text{(通过构建)}
\end{aligned}$$

每个父结点都对子结点画一条有向边, 就构成了一个**有向无环图**: 每个结点都表示一个随机变量, 有向边表示随机变量的依赖关系, 且每个结点附有一个条件概率分布$P(X_i|\pi(X_i)$ (根结点则是边缘概率分布$P(X_i)$).

> [!note]
> 由于有向无环图进行拓扑排序后, 其排序结果相当于规定了变量的顺序. 因此在构建网络时可以不考虑变量的顺序, 只要最终是无环的即可.

不同的变量顺序导致不同的网络结构，而不同的网络结构表示了联合分布的不同分解，而不同的分解则意味着不同的复杂度。选取**变量顺序**的原则有三种:
1. Smith(1989)认为应以模型的复杂度为标准。
2. Howard和Matheson(1984)认为变量顺序的选取应以条件概率评估的难易程度为标准。
3. Pearl(2000)提出应该用因果关系来决定变量顺序，原因在前，结果在后。
...
对于多变量总体, 一次<u>同时观测</u>$p$个变量, 进行$n$次观测. 对一个个体观测得到$X = (X_1, X_2, \dots, X_p)^T$. 这$p$个变量称为一个**样品**, $n$个样品形成一个**样本**.

|    变量    |  $X_1$   |  $X_2$   | $\cdots$ |  $X_p$   |
| :------: | :------: | :------: | :------: | :------: |
...
- $X_{(a)} = (x_{a1}, x_{a2}, \cdots, x_{ap})^T$, $a = 1, 2, \cdots, n$ 表示第 $a$ 个**样本**的观测值(行). 
- $X_j = (x_{1j}, x_{2j}, \cdots, x_{nj})^T$, $j = 1, 2, \cdots, p$ 表示对第$j$个**变量** $X_j$ 的 $n$ 次观测数值(列).

因此$X$是$n \times p$的**矩阵**:
$$X 
...
= (X_1, X_2, \cdots, X_p) 
= \begin{bmatrix} X_{(1)}^T \\ X_{(2)}^T \\ \vdots \\ X_{(n)}^T \end{bmatrix}$$

设$X_1, X_2, \cdots, X_p$为$p$个<u>随机变量</u>, 由它们组成的向量$X=(X_1, X_2, \cdots, X_p)^T$称为**随机向量**.

若$X \sim F$, 则有**多元分布函数**:
$$F(x)=F(x_1,x_2,\cdots,x_p)=P(X_1\leq x_1,X_2\leq x_2,...,X_p\leq x_p)$$

若存在一个非负的函数$f(\cdot)$, 使得 
$$ F(\mathbf{x}) = \int_{-\infty}^{x_1} \int_{-\infty}^{x_2} \cdots \int_{-\infty}^{x_p} f(t_1, t_2, \cdots, t_p) \, dt_1 \, dt_2 \cdots dt_p $$
对一切 $\mathbf{x} \in \mathbb{R}^p$ 成立, 则称$X$有**分布密度**$f(\cdot)$, 并称$X$为**连续型随机向量**. 
- $\forall \mathbf{x} \in R^p, f(\mathbf{x}) \geq 0$
- $\int_{R^p} f(\mathbf{x}) \, d\mathbf{x} = 1$

有两个可以是<u>不同维度</u>的多元随机变量$X$和$Y$, 如果$P(X\leq x,Y\leq y)=P(X\leq x)P(Y\leq y)$, 则$X$和$Y$**独立**.

## 多元随机变量的数字特征

定义**均值向量**$\mu$为 (要求所有$E(X_i)$存在):
$$E(X)=\begin{bmatrix}E(X_1)\\E(X_2)\\\vdots\\E(X_p)\end{bmatrix}=\begin{bmatrix}\mu_1\\\mu_2\\\vdots\\\mu_p\end{bmatrix}=\mu$$

- $E(AX)=AE(X)$
- $E(AXB)=AE(X)B$

对于$p$维的$X$和$q$维的$Y$, 定义其**协方差阵**为 第$i$行第$j$列为$cov(X_i,Y_j)$的$p \times q$的矩阵:
$$\begin{align}
cov(X,Y) 
&= E[(X - EX)(Y - EY)^T] \\
&= \big(cov(X_i,Y_j)\big) \\
...
&= (\sigma_{ij}) 
\end{align}$$
称$\vert \text{cov}(X, X) \vert$为$X$的**广义方差**. 若广义方差为$0$, 则可得**随机变量线性相关**(至少退化一维).

1. $D(AX) = AD(X)A^T = A\varSigma A^T$ 
2. $\text{cov}(AX, BY) = A\text{cov}(X, Y)B^T$ 
3. $E(X^T AX) = tr(A\varSigma) + \mu^T A\mu$ 

载样本语境下, 有类似于样本方差$S^2$的东西, 使得$\varSigma = E(\varSigma^\prime)$:
$$\varSigma^\prime = \frac 1 {n-1} (X - \overline X)(Y - \overline Y)^T$$

随机向量$X=(X_1, X_2, \cdots, X_p)^T$, 其协方差阵$\varSigma$都是对称阵, 同时总是半正定的. 大多数情形下是正定的。

$X$的**相关阵**定义为:
$$R=\left(corr(X_i,X_j)\right)=(r_{ij})_{p\times p}$$
$$r_{ij}=\frac{cov(X_i,X_j)}{\sqrt{D(X_i)}\sqrt{D(X_j)}},i,j=1,2,\cdots,p$$

为克服指标量纲不同的影响, 在应用时通常会对变量做标准化, 即:
$$ X_j^* = \frac{X_j - E(X_j)}{\sqrt{D(X_j)}}, \quad j = 1, 2, \cdots, p $$
$$ X^* = (X_1^*, X_2^*, \cdots, X_p^*)^T $$
显然:
$$E(X^*) = 0$$
标准化数据的**协方差矩阵**正好是<u>原数据的相关阵</u>:
$$D(X^*) = E[X^* X^{*T}] = corr(X) = R$$

如果在样本语境下, 不知道$E$和$D$, 则有(加上对样本方差的无偏纠正):
$$R^\prime = \frac{1}{n - 1} X^{*\prime} X^{*\prime T}$$

> [!info] 从<u>单变量</u>推导标准化数据的协方差矩阵正好是原数据的相关阵
> 由$E(X^*) = 0$可知:
> $$cov(X_i^*, X_j^*) = E(X_i^*X_j^*)$$
> 
> 而由$X^*$定义直接推得:
> $$E(X_i^* X_j^*) = corr(X_i, X_j)$$
> 
> 从而得到:
> $$cov(X_i^*, X_j^*) = corr(X_i, X_j)$$
> 
> 因此有结论:
> $$D(X^*) = \text{corr}(X) = R$$
...
其中$\mu$是均值向量, $V$是协方差矩阵, 且$V>0$. 当$|V| = 0$时, $X$分布为退化的正态分布.

另一个等价的定义: 若对任何非零向量$a \in R^p$, $X$的线性组合$a^T X$服从一元正态分布$N(a^T \mu, a^T V a)$, 则称$X$服从$p$元正态分布$N_p(\mu, V)$.
...
4. $X$为$p$维正态随机向量的充要条件为对任一$p$维向量$c$, $c^TX$是一维正态随机变量.
5. 设$X=(X_1^T, X_2^T)^T$为多维正态随机向量, 则$X_1$与$X_2$互不相关的充要条件是$X_1$与$X_2$相互独立. 注: 若$Cov(X, Y)=0$，则称$X$与$Y$互不相关.
6. 设$X \sim N_p(\mu, V)$, 则$rank(V)=m$的充要条件是存在$m \times p$的矩阵$B(BB^T = V)$使得$X = BY+\mu$, 其中$Y \sim N_m(0, I_m)$.
7. 若$X \sim N_p(\mu, V)$, 且$\vert V\vert\neq0$, 则 $\eta \triangleq (X - \mu)^T V^{-1}(X - \mu) \sim \chi^2(p)$.
...
**样本离差阵**:
$$S=\sum_{k=1}^n(X_k-\overline{X})(X_k-\overline{X})^T$$

- $\overline X$是$\mu$的极大似然估计
- $\frac 1 n S$是$V$的极大似然估计

- $\overline X$是$\mu$的UMVUE
- $\frac 1 {n-1} S$是$V$的UMVUE

- $\overline{X}\sim N_p\left(\mu,\frac1nV\right)$, 且$\overline{X}$与$S$相互独立.
- $S\sim W_p(n-1,V)$, 即自由度为$n-1$的Wishart分布.
...
\\ 0, & \text{others} 
\end{cases}$$
则有$E(X) = \frac {a + b} {2}$, $D(X) = \frac{(b - a)^2}{12}$.


## 两点（0-1）（Bernoulli）分布

若$X \sim B(1, p)$，则$X$符合两点分布。$X$有$p$的概率为$0$，有$1-p$的概率为$1$。
...
进行若干次$B(1, p)$的伯努利试验, 直到成功一次所需的试验次数为$X$, 则$X$服从几何分布, 其分布列为:
...
**可加性**: $X_1\sim P(\lambda_1)$, $X_2\sim P(\lambda_2)$, $X_1$和$X_2$相互<u>独立</u>,则 $X_1+X_2\sim P(\lambda_1+\lambda_2)$.

> [!note]
> 泊松分布只是表示一个值$k$的概率, 如果要求一个范围的话就需要求和. 例如:
> $$P(X \geq 3) = \sum_{k=3}^{+\infty} \frac{\lambda^k}{k!}e^{-\lambda} = 1 - \sum_{k=0}^{2} \frac{\lambda^k}{k!}e^{-\lambda}$$
...
正态分布进行线性变换与组合后还是正态分布, 可以分别算均值和方差, 然后就能直接写出新正态分布.

如果$X\sim N(\mu,\sigma^2)$ , 则$aX+b\sim N(a\mu+b,(a\sigma)^2)$. 

如果$X\sim N\left(\mu,\sigma^{2}\right)$，则$Y=\frac{X-\mu}{\sigma}\sim N\left(0,1\right)$。
...
$$\frac{n\hat\sigma^{2}}{\sigma^{2}}\sim\chi^{2}\left(n-1\right)$$
因此也有:
$$\frac{\left(n-1\right)S^{2}}{\sigma^{2}}\sim\chi^{2}\left(n-1\right)$$

> [!note]
> 注意, 下面这个是完完全全的原教旨$\chi^2(n)$分布, 不要弄混:
...
设$X_1,X_2,\cdotp\cdotp\cdotp,X_n$是相互独立的随机变量, 且$X_i\sim N(0,1)$, 则称随机变量
...
> [!note]
> 记忆：“正态分布”除以“正态分布的平方平均数的根号”，最后量纲是1。
...
设$X\sim N(\mu,\sigma^{2})$，$\frac{Y}{\sigma^{2}}\sim\chi^{2}(n)$,且$X$与$Y$相互独立，则有(上下方差抵消)：
...
若总体$X \sim B(\alpha, \beta)$, 其中$\alpha, \beta > 0$, 则其概率密度函数为:
$$f(x;\alpha, \beta) = \frac{1}{B(\alpha,\beta)}x^{\alpha-1}(1-x)^{\beta-1}$$
其中分母的$B(\alpha,\beta)$是[B函数](概率论与数理统计/概率论与数理统计.md#B%20函数).
...
定义为：对于$\forall\varepsilon>0$ 有（越界是趋向小概率的）：
...
因此依概率收敛时，**统计量可能是不收敛于被估值的**，因为[收敛的定义](微积分/微积分.md#极限)要求对任意界限总是能够找到数列上的$[a, +\infty)$项必然在此界限当中；但<u>依概率收敛中统计量的界限没有被限制</u>。
...
即当$n$趋于无穷大时，**必然**有统计量**收敛**于被估值。

当$n$不断趋于无穷大时，统计量随之不断变化形成一个队列，而几乎必然收敛则要求这个队列必然收敛于被估值。这就要求统计量在$n$变大的过程中要按照收敛的定义去贴近被估值，即使是无穷小概率也不能出现范围飘忽的值。
...
定义为：若$\{X_n\}$和$\{X_0\}$的**分布函数**分别为$\{F_n(\cdot)\}$的$\left\{F_0(\cdot)\right\}$，若在$F_0(x)$连续处，对于所有的$x\in R$都有
...
若有$P\begin{Bmatrix}Z>z_\alpha\end{Bmatrix}=\alpha,0<\alpha<1$, 则$z_a$为随机变量$Z$的上$\alpha$分位点.

若有$P\left\{Z<z_{1-\alpha}\right\}=\alpha,0<\alpha<1$, 则$z_a$为随机变量$Z$的下$\alpha$分位点.

> [!tip]
> 可见符号$z_x$定义为上$x$分位点.

> [!warning]
> 有时候使用$z_x$表示下$x$分位点, 或简称 **分位数**. 在本笔记里也较多这么使用.
...
$E_\theta(g(x, \theta))$表示$E$最终为$\theta$的函数, 因此这个$E$是对$x$求期望, 即$\int x \cdot g(x, \theta) dx$.
...
$t$在这里仅仅作为表达形式. 或者说因为联合分布列本来就是求一个完全具体的情况的概率，只受随机变量影响。$t$的加入不会引入额外的随机变量，只会影响$X_1, ..., X_n$的取值情况；而$X_1, ..., X_n$的任意取值的概率都只会被表示成$10 \sum_{i=1}^n{x_i}$，因此$t$可以直接代入。
...
令$u = \frac {z^2} 2$，则：
$$E(|Z|) = \sqrt{\frac{2}{\pi}}\int_0^\infty e^{-u}du 
= \sqrt{\frac{2}{\pi}}$$
