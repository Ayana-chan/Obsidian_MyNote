...
> 下面这个式子使用[协方差](概率论与数理统计/概率论与数理统计.md#协方差与相关系数)更好理解。
...
$$L(\theta;x_1,x_2,\cdots,x_n)=\prod_{i=1}^np(x_i,\theta)$$
称为**似然函数（Likelihood Function）**，简记为$L(\theta)$。其定义域为参数空间$\Theta$。

> [!tip]
> $L(\theta)$的$x_i$都被视为已知数，即会使用具体的样本观察值进行代入，因此其自变量只有参数$\theta$。

如果存在$\widehat{\boldsymbol{\theta}}\in\Theta$,使$L(\boldsymbol{\theta})$达到最大，即
$$L(\widehat{\theta}(x);x)=\sup_{\theta\in\Theta}\{L(\theta,x)\}=\sup_{\theta\in\Theta}\left\{\prod_{i=1}^np(x_i,\theta)\right\}$$
则称此$\widehat{\boldsymbol{\theta}}(x)$为参数$\theta$的**极大似然估计（Maximum Likelihood Estimate, MLE）**。它是样本观察值$x_i$的函数。

相应的，称$q\left(\widehat{\theta}(x)\right)$为$q(\boldsymbol{\theta})$的MLE。

若参数空间只有有限元，则可以通过**穷举**来完成估计。否则，可以先获取**对数似然函数**（通过对$L(\theta)$取对数来化乘为加），然后通过<u>求导</u>（对各$\theta$内分量求偏导）来完成估计，此时问题转化为求解**似然方程（Likelihood Function）**：
...
设$g(t)$是定义在统计量$T(x)$的值域上的任一实值函数，如果对任意$\theta\in\Theta$都有$E_\theta(g(T))=0$成立时，必成立$g(T)\equiv0$,则称统计量$T(x)$是**完全的(Complete)**。
...
设$x=(x_1,x_2,\cdotp\cdotp\cdotp,x_n)$是来自总体$\{P_\theta,\theta\in\Theta\}$
的一个样本，其密度函数可表示为
$$p(x,\theta)=c(w)h(x)\exp\left\{\sum_{k=1}^mw_kT_k(x)\right\}$$
...
如果Ω有内点，则统计量$(T_1(x),\cdotp\cdotp\cdotp,T_m(x))$是**完全充分的**。

---

**Lehmann-Scheffe 定理**：设$S(x)$是完全充分统计量，$\varphi(x)$是$q(\theta)$无偏估计，则$T(x)=E_\theta(\varphi(x)|S(x))$是$q(\theta)$的UMVUE。进一步如果对所有$\theta\in\Theta,Var_\theta(T(x))<\infty$,则$T(x)$是$q(\theta)$唯一的UMVUE。

Lehmann-Scheffe 定理给出了两种通过完全充分统计量$T(x)$寻找UMVUE的方法：
1. 若$h(T(x))$是$q(\theta)$无偏统计量，则$h(T(x))$也是$q(\boldsymbol{\theta})$的UMVUE。即寻找完全充分统计量的函数使之成为$q(\boldsymbol{\theta})$的无偏估计。
2. 若能获得$q(\theta)$的一个无偏估计量$\varphi(x)$,则$E(\varphi(x)|T(x))$就是$q(\theta)$的UMVUE。（这个方法更容易）
