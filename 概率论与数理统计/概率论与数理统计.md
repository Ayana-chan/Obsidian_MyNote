...
一定有$\Theta_0 \cap \Theta_1 = \varnothing$，但不要求$\Theta_0 \cup \Theta_1 = \Theta$。
...
> [!info]
> 普遍性的，当样本容量$n$固定时，要减少犯第一类错误概率，就会增大犯第二类错误的概率；反之，若要减少犯第二类错误概率，就会增大犯第一类错误的概率。即就是说<u>当样本容量固定时，不可能同时减少犯两类错误的概率</u>。

**Neyman-Pearson检验原理**就是控制**犯第一类错误的概率**<u>在给定的范围内</u>，寻找检验使得**犯第二类错误的概率**<u>尽可能的小</u>（等价于让检验的**势尽可能的大**）。这样就是在给定一个较小的数$\alpha(0<\alpha<1)$（一般取为0.01,0.05,0.1等），在满足
...
对给定的$\alpha\in(0,1)$，若检验函数$\varphi(x)$对所有的参数$\theta\in\Theta_0$，满足$E_{\theta}(\varphi(x))\leqslant\alpha$，则称$\varphi(x)$是一个显著性水平为$\alpha$的检验函数，简称**水平为$\alpha$的检验**。
...
> [!note]
> $P_{\theta_0} \{ u \geq c \} \leq \alpha$可以表示成$u_{1 - \alpha}$，表示在$u$的分布函数上取值为$1-\alpha$的点（比这个点大的区间的概率为$\alpha$）。
...
> [!tip]
> 此时分别求在$\theta\in\Theta$和$\theta\in\Theta_0$的情况下$\theta$的**极大似然估计**$\hat\theta$和$\hat\theta_0$，然后分别在分子和分母代入就能去掉$sup$。


## 最优势检验 & Neyman-Pearson 引理
...
**Neyman-Pearson 引理（NP引理）**：对<u>上述检验问题</u>，对给定$\alpha \in (0,1)$，有：

（1）存在常数$k\geq0$及检验（**<u>式2</u>**）：
$$\varphi(x)=\begin{cases}1,&L(x)>k\\0,&L(x)<k\end{cases}$$
检验$\varphi(x)$是水平$\alpha$的**MPT**且满足$E_{\theta_0}(\varphi(x))=\alpha$（**<u>式3</u>**）。

(2) 
- 如果$\boldsymbol\varphi(x)$是水平为$\alpha$的MPT，则必存在常数$k\geq0$，使得$\varphi(x)$满足<u>式2</u>。
...
> - 如果$k$太小了，那么必然有$r<0$，是非法的。因为此时$P_{\theta_0}\{L(x)> k\} > \alpha$，于是不得不让$r$位负来使$E_{\theta_0}(\varphi(x))$回调到$\alpha$。
...
对检验问题：
$$H_0:\theta\in \Theta_0,H_1:\theta\in \Theta_1.$$
设$\boldsymbol\varphi^*(\boldsymbol{x})$是水平为$\alpha$的检验；如果对任一水平为$\alpha$的检验$\varphi\left(x\right)$,有不等式
...
总体的概率密度函数记为$p(x|\theta)$, 即$\theta$为某一值的情况下的<u>条件概率函数</u>.
...
设$p(\theta)$是待估计参数0的先验概率，0取值与样本集$D=\{x_1,x_2,...,x_N\}$有关, 则有流程:样本信息 $p(D|\theta)$ $+$ 先验信息 $p(\theta)$ $\rightarrow$ 贝叶斯定理 $\rightarrow$ 后验信息 $p(\theta|D)$ $\rightarrow$ 统计推断

设样本取值空间为$E^d$, 参数$\theta$取值空间为$\Phi$, $\lambda(\widehat{\theta},\theta)$是$\widehat{\theta}$作为$\theta$的<u>估计量</u>时的**损失函数**，定义样本x下的**条件风险**为：
$$R(\widehat{\theta}|x)=\int_{\Phi}\lambda(\widehat{\theta},\theta) p(\theta|x)\boldsymbol{d}\theta$$
则**总期望风险**为：
$$R=\int_{E^d} R(\widehat{\theta}|x) p(x)dx$$
在有限样本集合$D=\{x_1,x_2,...,x_N\}$的情况下, 求$R$最小等价于求$R(\widehat{\theta}|x)$最小。即：
$$\theta^*
=argmin R(\widehat{\theta}|D)
=argmin\int_\Phi\lambda(\widehat{\theta},\theta)p(\theta|D)d\theta$$
如果使用**平方损失函数**$\lambda(\widehat{\theta},\theta) = (\theta - \hat\theta)^2$, 则有:
...
> [!note]
> 求$p(\theta|D)$时, $p(D|\theta)p(\theta)$的与$\theta$无关的乘数都会被除法抵消.

> [!note]
> 求伯努利相关的$\theta^*$时经常会凑出[B函数](概率论与数理统计/概率论与数理统计.md#B%20函数)的形式.

> [!tip]
> 给出几个观察值, 去求贝叶斯估计时, 就相当于提供了有具体值的样本.
...
&= (\sigma_{ij}) 
\end{align}$$
称$\vert \text{cov}(X, X) \vert$为$X$的**广义方差**.
...
$$D(X^*) = E[X^* X^{*T}] = corr(X) = R = \frac{1}{n - 1} X^{*T} X^*$$
...
进行若干次$B(1, p)$的伯努利试验, 直到成功一次所需的试验次数为$X$, 则$X$服从几何分布, 其概分布列为:
...
$X_1\sim P(\lambda_1)$, $X_2\sim P(\lambda_2)$, $X_1$和$X_2$相互<u>独立</u>,则 $X_1+X_2\sim P(\lambda_1+\lambda_2)$.
...
因此依概率收敛时，**统计量可能是不收敛于被估值的**，因为[收敛的定义](微积分/微积分.md#极限)要求对任意界限总是能够找到数列上的$[a, +\infty)$项必然在此界限当中；但<u>此时的统计量的界限没有被界定</u>。
...
当$n$不断趋于无穷大时，统计量随之不断变化形成一个队列，而几乎必然收敛则要求这个队列必然收敛于被估值。这就要求统计量在$n$变大的过程中要按照收敛的定义去贴近被估值，即使是无穷小概率也不能出现上界飘忽的值。
...
$E_\theta(g(x, \theta))$表示其为$\theta$的函数, 因此这个$E$是对$x$求期望, 即求$\int x \cdot g(x, \theta) dx$.
